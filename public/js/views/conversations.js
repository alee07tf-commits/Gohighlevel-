...
export async function renderConversations(view, rest = []) {
  ...
  <div class="page-header"><h1>${t('Conversaciones', 'Conversations')}</h1>
    <span class="badge indigo">${convs.filter((c) => c.unread > 0).length} ${t('sin leer', 'unread')}</span></div>
          ...
          ? convs
              .map(
                (c) => `<div class="conv-item ${c.id === activeId ? 'active' : ''}" data-id="${c.id}">
                  ...
                  <div class="c-preview">${c.last_channel ? `[${c.last_channel}] ` : ''}${esc(c.last_body || t('Aún no hay mensajes', 'No messages yet'))}</div>
                </div>`
              )
              .join('')
          : `<div class="empty">${t('Aún no hay conversaciones.', 'No conversations yet.')}<br>${t('Envía un mensaje a un contacto para empezar una.', 'Message a contact to start one.')}</div>`
      }
    </div>
